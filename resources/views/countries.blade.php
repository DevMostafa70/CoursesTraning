        ...
        @if (@isset($data) and !@empty($data))
            @foreach ($data as $info)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('country.edit', $info->id) }}" class="button"
                            style="background-color: #04AA6D; padding:10px;"> تعديل</a>
                        <form action="{{ route('country.destroy', $info->id) }}" method="Post">
                            @csrf
                            @method('DELETE')
                            <button class="button"
                                style="background-color: #aa0704; padding:10px;"> حذف</button>
                    </td>
                </tr>
            @endforeach
        @endif
